...
def plot_team_ticket_totals(team_name, data, axis, writer, show_legend, show_ylabel):
    ...
    # Plot data in graph
    ticket_total.plot.line(rot=0, y="Total", x="Resolved", color={"Total": "purple"}, ax=axis)
...
def load_csv(filename, teams):
    ...
    with pd.ExcelWriter(file_xlsx) as writer:
        ...
        for team in teams:
            plot_team_ticket_totals(team, data, axes if number_of_teams == 1 else axes[axis_index], writer, axis_index == number_of_teams - 1, axis_index == 0)
            axis_index += 1
...
def remove_extension_from_name(filename):
...
def main():
    ...
    args = sys.argv[1:]
    if len(args) == 1:
        load_csv(remove_extension_from_name(args[0]), teams)
    elif len(args) == 3:
        if args[1] == "-t":
            load_csv(remove_extension_from_name(args[0]), [args[2]])
        else:
            print("Unknown args: " + str(args))
            print("Usage: report.py '<file_name>' -t '<team_name>'")
    else:
        print("Unknown args: " + str(args))
